...
- **Customer** 進入時先選擇是否為會員；非會員可直接聊需求，會員輸入編號（例如：`M10001`、`M10005`）後後台即時讀取資料庫並顯示辨識結果。
...
4. Start Postgres (default `localhost:8002`, user `db_admin`, password `db_password`, db `marketing_tools`). The repo ships with `docker compose up postgres` for a ready-to-run instance.
5. Start backend API: `cd backend && uvicorn main:app --reload --port 4000`
6. Launch portals (individual terminals):
...
- **Postgres** (`localhost:8002`, `db_admin/db_password`, DB name `marketing_tools`) stores:
  - `members`（10 筆模擬資料，ID 例如 `M10001`-`M10010`）與 `purchases`（100 筆最近 6 個月的隨機交易）。
  - `conversation_messages`（所有聊天紀錄）。
  - `documents` 與 `document_chunks`：原始檔案直接以 `BYTEA` 保存，chunk 與 embedding 則以 JSONB 保存，供語意搜尋使用。
- 專案啟動時會自動建立資料表並初始化樣本資料；若要清空可直接刪除或重建該資料庫。
- `docker compose up postgres` 會啟動同樣設定的資料庫容器，也可改指向既有的 Postgres，僅需更新 `backend/.env.local` 中的連線資訊或 `DATABASE_URL`。
...
1. 複製並設定 env（特別是 `backend/.env.local` 內的 `CHATGPT_API_KEY` 與資料庫設定）。
2. 啟動容器：`make up`（Customer 3005 / Agent 3006 / Backend 4000 / Postgres 8002）。
3. 需要看 log：`make logs`
4. 停止並清除：`make down`
